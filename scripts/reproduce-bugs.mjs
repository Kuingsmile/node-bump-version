...
function fixture(name, initialVersion = '1.0.0', lockfileVersion = 3) {
  ...
  const pkg = { name: 'reproduce-fixture', version: initialVersion, repository: 'https://github.com/example/reproduce-fixture.git' }
  writeJson(join(cwd, 'package.json'), pkg)
  writeJson(join(cwd, 'package-lock.json'), {
    name: pkg.name, version: initialVersion, lockfileVersion,
  ...
  return new Promise((resolveResult, reject) => {
    ...
    const receive = chunk => {
      ...
      if (confirm && !answered && output.includes('is it right?')) {
        answered = true
        setImmediate(() => { if (!child.stdin.destroyed) child.stdin.write('y\n') })
      }
    }
...
const cases = [
  {
    id: 1, severity: 'P1', title: 'Shell injection and paths containing spaces',
    ...
    async run() {
      ...
      let failure
      try { await api.mainLifeCycle({ _: [], path: spaced, changelog: false }, '1.0.0', '1.0.1') }
      catch (error) { failure = error }
      const cwd = fixture('shell-filename')
      process.chdir(cwd)
      const marker = 'REPRODUCE_HARMLESS_ECHO'
      const file = process.platform === 'win32'
        ? `CHANGE&echo ${marker}&rem .md`
        : `CHANGE;echo ${marker};#.md`
  ...
  {
    id: 2, severity: 'P1', title: 'README npm dry command performs a real release',
    correct: 'The documented dry command leaves the version, HEAD, and tags unchanged.',
    buggy: { documentedUnsafeCommand: true, version: '1.0.1', releaseTagCreated: true, commitCreated: true, properlyForwardedDryIsClean: true },
    async run() {
      const npm = findNpm()
      ...
      const properlyForwardedDryIsClean = git(cwd, 'status', '--porcelain') === ''
        && git(cwd, 'rev-parse', 'HEAD') === head && git(cwd, 'tag', '--list') === 'v1.0.0'
      ...
      return {
        ...
        version: version(cwd), releaseTagCreated: git(cwd, 'tag', '--list', 'v1.0.1') === 'v1.0.1',
        commitCreated: git(cwd, 'rev-parse', 'HEAD') !== head, properlyForwardedDryIsClean
      }
    }
  },
  {
    id: 3, severity: 'P1', title: '--path takes the version from the caller',
  ...
  {
    id: 4, severity: 'P1', title: 'dry:false tags a package whose version was not bumped',
    ...
    async run() {
      ...
      return {
        workingVersion: version(cwd), taggedVersion: JSON.parse(git(cwd, 'show', 'v1.0.1:package.json')).version,
        changelogUpdated: readChangelog(cwd).includes('1.0.1')
      }
    }
  },
  {
    id: 5, severity: 'P1', title: 'Invalid release type writes an empty version and tag v',
    ...
    async run() {
      ...
      return {
        exitCode: result.code, version: version(cwd), invalidTagCreated: git(cwd, 'tag', '--list', 'v') === 'v',
        commitCreated: git(cwd, 'rev-parse', 'HEAD') !== head
      }
    }
  },
  {
    id: 6, severity: 'P2', title: 'Changelog is written in the caller directory',
  ...
  {
    id: 7, severity: 'P2', title: 'Relative --path is resolved twice during Git operations',
    ...
    async run() {
      ...
      return {
        failedWithPathspec: result.code !== 0 && /pathspec/.test(result.output), targetVersion: version(target),
        headUnchanged: git(target, 'rev-parse', 'HEAD') === head, filesLeftDirty: git(target, 'status', '--porcelain') !== ''
      }
    }
  },
  {
    id: 8, severity: 'P2', title: 'Lockfile v2/v3 root-package version stays stale',
    correct: 'Both lockfile version fields match package.json after a bump.',
    buggy: [2, 3].map(lockfileVersion => ({ lockfileVersion, manifest: '1.0.1', topLevel: '1.0.1', rootPackage: '1.0.0' })),
    async run() {
      const observations = []
      for (const lockfileVersion of [2, 3]) {
        ...
        observations.push({ lockfileVersion, manifest: version(cwd), topLevel: lock.version, rootPackage: lock.packages[''].version })
      }
      return observations
    }
  },
  {
    id: 9, severity: 'P2', title: 'Feature commits receive a patch recommendation',
    ...
    async run() {
      ...
      const { default: recommended } = await import('../dist/conventional-changelog-node/conventional-recommended-bump.js')
  ...
  {
    id: 10, severity: 'P2', title: 'Commit message hook does not enforce commitlint',
    correct: 'The configured commit-msg hook rejects an invalid message.',
    buggy: { legacyConfigurationPresent: true, invalidCommitAccepted: true, directLintRejects: true, validMessageAccepted: true, commitMsgHookExists: false },
    async run() {
      ...
      const lint = message => spawnSync(process.execPath, [join(project, 'node_modules/@commitlint/cli/cli.js')], {
      ...
      assert.match(invalid.stdout + invalid.stderr, /type-empty/, 'Commitlint must reject the message for the expected reason')
      git(cwd, 'add', 'package.json')
      const commit = spawnSync('git', ['commit', '-m', 'invalid message'], { cwd, encoding: 'utf8', timeout: 15000 })
      assert.ok(commit.status === 0 || /type-empty/.test(commit.stdout + commit.stderr), 'Commit must succeed or fail specifically on commitlint validation')
      const invalidCommitAccepted = commit.status === 0 && git(cwd, 'log', '-1', '--format=%s') === 'invalid message'
      const validCommit = spawnSync('git', ['commit', '--allow-empty', '-m', ':bug: Fix(core): handle input'], { cwd, encoding: 'utf8', timeout: 15000 })
      ...
      return {
        ...
        directLintRejects: invalid.status !== 0, validMessageAccepted: validCommit.status === 0,
        commitMsgHookExists: existsSync(join(cwd, '.husky/commit-msg'))
      }
    }
  },
  {
    id: 11, severity: 'P3', title: 'Help requires a package and startup errors exit successfully',
    ...
    async run() {
      ...
      return {
        helpExitCode: help.code, helpMissing: !help.output.includes('Usage') && help.output.includes('not found'),
        missingPackageExitCode: missing.code, malformedPackageExitCode: malformed.code
      }
    }
  }
]
...
function worker(id, directory, env) {
  return new Promise(resolveResult => {
    const child = fork(script, ['--worker', String(id), '--root', directory], {
      cwd: project, env, stdio: ['ignore', 'pipe', 'pipe', 'ipc']
    })
...
async function main() {
  const { values } = parseArgs({ options: {
    help: { type: 'boolean', short: 'h' }, case: { type: 'string', multiple: true },
    'skip-build': { type: 'boolean' }, 'temp-dir': { type: 'string' },
    worker: { type: 'string' }, root: { type: 'string' }
  } })
  if (values.help) {
  ...
  if (values.worker) {
    ...
    if (id === 0) process.send(await baseline())
      ...
      process.send({ id, severity: item.severity, title: item.title,
        status: isDeepStrictEqual(observed, item.buggy) ? 'REPRODUCED' : 'NOT REPRODUCED',
        correctBehavior: item.correct, observed })
    }
    return
  }
  ...
  assert.ok(!/[\s&;|<>^%!"'`$()]/.test(parent), 'Use --temp-dir with a path without spaces or shell characters to isolate the filename bug')
  ...
  const env = Object.fromEntries(Object.entries(process.env).filter(([key]) => !/^(GIT_|HUSKY$|npm_config_)/i.test(key)))
  Object.assign(env, {
    GIT_CONFIG_NOSYSTEM: '1', GIT_CONFIG_GLOBAL: join(root, 'gitconfig'), GIT_TERMINAL_PROMPT: '0', HUSKY: '1',
    ...
    XDG_CONFIG_HOME: join(root, 'xdg-config'), NO_COLOR: '1'
  })
